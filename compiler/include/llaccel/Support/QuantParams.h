...
// the NUMERICS.md line it implements.
#pragma once
...
/// Throws if r <= 0 or S would be negative (r >= 2^31).
inline MulShift normalizeMulShift(double r) {
  if (!(r > 0.0) || !std::isfinite(r))
    throw std::runtime_error("normalizeMulShift: ratio must be positive and finite");
  int e = 0;
  ...
  if (S < 0)
    throw std::runtime_error("normalizeMulShift: ratio too large (S < 0)");
...
inline int64_t i16Exponent(double absmax) {
  if (!(absmax > 0.0))
    return -15;
  double e = std::ceil(std::log2(absmax / 32767.0));
...
inline int16_t quantI16(double v, int64_t e) {
  long long q = std::llround(std::ldexp(v, int(-e)));
  if (q > 32767) q = 32767;
  if (q < -32768) q = -32768;
  return int16_t(q);
}
...
inline MulShift gemmRq(double s_a, double s_w, bool outI8, int64_t e_out, double s_out) {
...
inline int32_t gemmBias(double b, double s_a, double s_w) {
  long long v = std::llround(b / (s_a * s_w));
...
inline MulShift quantParams(int64_t e_x, double s_y) {
...
///             calibrated typical inv = C / isqrt(K * rms_q^2 + eps_t) < 65535/4,
///             where rms_q = (absmax_x / 4) / 2^e_x (heuristic: rms ~= absmax/4,
///             i.e. the abs-max is about four standard deviations)
///   sh_post = R - e_g + e_y   (must be in [0, 63])
inline RmsNormParams rmsnormParams(int64_t K, double eps, int64_t e_x, int64_t e_g, int64_t e_y,
                                   double absmax_x, const std::string &name) {
  RmsNormParams p{};
  double epsT = std::llround(eps * double(K) * std::ldexp(1.0, int(-2 * e_x)));
  if (epsT >= std::ldexp(1.0, 48))
    throw std::runtime_error("rmsnorm " + name + ": eps_t does not fit in 48 bits");
  p.eps_t = int64_t(epsT);
  double rmsQ = (absmax_x / 4.0) / std::ldexp(1.0, int(e_x));
  double rTyp = std::floor(std::sqrt(double(K) * rmsQ * rmsQ + double(p.eps_t)));
  if (rTyp < 1.0) rTyp = 1.0;
  int64_t R = -1;
  for (int64_t cand = 31; cand >= 0; --cand) {
    double C = std::llround(std::ldexp(std::sqrt(double(K)), int(cand)));
  ...
  if (R < 0)
    throw std::runtime_error("rmsnorm " + name + ": no R in [0,31] keeps inv below 65535/4 "
                             "(input abs-max " + std::to_string(absmax_x) + " too small for e_x)");
  p.R = R;
  p.sh_post = R - e_g + e_y;
  if (p.sh_post < 0 || p.sh_post > 63)
    throw std::runtime_error("rmsnorm " + name + ": sh_post = " + std::to_string(p.sh_post) +
                             " outside [0, 63]");
  return p;
}
...
inline SiluParams siluParams(int64_t e_x, int64_t e_y, const std::string &name) {
  SiluParams p{int64_t(1) << 30, 30 - (e_x + 12), 16 + e_y - e_x};
  if (p.Si < 0 || p.Si > 63)
    throw std::runtime_error("silu " + name + ": Si = " + std::to_string(p.Si) + " outside [0, 63]");
  if (p.sh_out < 0 || p.sh_out > 63)
    throw std::runtime_error("silu " + name + ": sh_out = " + std::to_string(p.sh_out) +
                             " outside [0, 63]");
  return p;
}

/// MUL: sh = e_a + e_b - e_y (in [0, 63]).
inline int64_t mulShift(int64_t e_a, int64_t e_b, int64_t e_y, const std::string &name) {
  int64_t sh = e_a + e_b - e_y;
  if (sh < 0 || sh > 63)
    throw std::runtime_error("mul " + name + ": sh = " + std::to_string(sh) + " outside [0, 63]");
  return sh;
}

/// ADD: sh_b = e_b - e_a >= 0, e_y = e_a.
inline int64_t addShift(int64_t e_a, int64_t e_b, const std::string &name) {
  int64_t sh = e_b - e_a;
  if (sh < 0 || sh > 63)
    throw std::runtime_error("add " + name + ": sh_b = " + std::to_string(sh) +
                             " outside [0, 63] (b must not have a smaller exponent than a)");
  return sh;
}
...
inline AttnParams attnParams(double s_q, double s_k, double s_v, double s_out, int64_t D) {
  AttnParams p;
  p.s = normalizeMulShift(256.0 * s_q * s_k / std::sqrt(double(D)));
  p.o = normalizeMulShift(s_v / (256.0 * s_out));
  return p;
}
