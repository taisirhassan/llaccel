...
int main(int argc, char** argv) {
  ...
  while (cases.size() < 12000) {
    ...
    uint32_t a32 = uint32_t(rng()) >> (rng() % 33);
    uint32_t b24 = uint32_t(rng() % (1u << 24)) >> (rng() % 25);
    uint32_t b32 = uint32_t(rng()) >> (rng() % 33);
  ...
  for (const Case& c : cases) {
    ...
    run(top, cycle);
    top.start = 0;
    // expect isqrt done after 24 iteration cycles (+1), udiv after 32 (+1)
    int sq_lat = -1, d24_lat = -1, d32_lat = -1;
    for (int k = 1; k <= 40; ++k) {
      run(top, cycle);
  ...
  std::printf("tb_math: %llu operand sets (isqrt48, udiv 32/24, udiv 32/32 each), %llu failures\n",
